...
const ProjectCard = ({ project }) => {
  ...
  return (
    <Link
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-slate-700/60 bg-base-800/50 shadow-[0_0_25px_-12px_var(--accent)] transition-all duration-300 ease-in-out hover:-translate-y-1 hover:border-[var(--accent)]/60 hover:shadow-[0_0_40px_-8px_var(--accent)]"
    >
      {/* Bottom accent bar */}
      <div
      ...
      {/* Image banner */}
      <div className="relative h-40 overflow-hidden shrink-0 bg-base-700 sm:h-44">
        <img
          src={image}
          alt={title}
      ...
      {/* Content — flex-1 + flex-col so the footer link can be pinned to the bottom */}
      <div className="flex flex-col flex-1 p-6">
        ...
        {/* Pinned to the bottom of every card via mt-auto, regardless of
            how much content sits above it — keeps this row aligned across
            a grid of cards with varying description lengths. */}
        <div
          style={{ color: "var(--accent)" }}
